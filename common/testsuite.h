...
static void mavlink_test_erl_quad_states(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_ERL_QUAD_STATES >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_erl_quad_states_t packet_in = {
        93372036854775807ULL,{ 73.0, 74.0, 75.0 },{ 157.0, 158.0, 159.0, 160.0 },{ 269.0, 270.0, 271.0 },{ 353.0, 354.0, 355.0 },{ 437.0, 438.0, 439.0, 440.0 },{ 549.0, 550.0, 551.0, 552.0 }
    };
    mavlink_erl_quad_states_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.timestamp = packet_in.timestamp;
        
        mav_array_memcpy(packet1.position, packet_in.position, sizeof(float)*3);
        mav_array_memcpy(packet1.orientation, packet_in.orientation, sizeof(float)*4);
        ...
        mav_array_memcpy(packet1.controls_scaled, packet_in.controls_scaled, sizeof(float)*4);
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_ERL_QUAD_STATES_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_ERL_QUAD_STATES_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_erl_quad_states_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_erl_quad_states_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_erl_quad_states_pack(system_id, component_id, &msg , packet1.timestamp , packet1.position , packet1.orientation , packet1.velocity , packet1.angular_velocity , packet1.controls , packet1.controls_scaled );
    mavlink_msg_erl_quad_states_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_erl_quad_states_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.timestamp , packet1.position , packet1.orientation , packet1.velocity , packet1.angular_velocity , packet1.controls , packet1.controls_scaled );
    mavlink_msg_erl_quad_states_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_erl_quad_states_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_erl_quad_states_send(MAVLINK_COMM_1 , packet1.timestamp , packet1.position , packet1.orientation , packet1.velocity , packet1.angular_velocity , packet1.controls , packet1.controls_scaled );
    mavlink_msg_erl_quad_states_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_erl_quad_states(system_id, component_id, last_msg);
